...
def trigger_model_update():
    """
    Notifies the main server to start the model list update process.
    """
    try:
        logging.info("Sending request to main server to update model list...")
        ...
        if response.json().get("status") == "success":
            ...
            logging.info("Please ensure the LMArena page is open; the script will automatically extract the latest model list from the page.")
...
if __name__ == "__main__":
    ...
    # Automatically exit after script execution
    time.sleep(2)
